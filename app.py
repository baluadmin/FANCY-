...
if role == "Owner / Admin":
    ...
    if st.session_state.user_role != "Owner":
        if not st.session_state.password_verified:
            owner_pass = st.sidebar.text_input("1. Enter Admin Password:", type="password", key="owner_pass_input")
            ...
            mobile_number = st.sidebar.text_input("2. Enter Mobile Number:", max_chars=10, key="owner_mobile_input")
            if st.sidebar.button("Send OTP to Mobile"):
                if len(mobile_number) == 10 and mobile_number.isdigit():
                    ...
                    st.sidebar.warning("⚠️ Please enter a valid 10-digit mobile number.")

        elif st.session_state.otp_sent:
            entered_otp = st.sidebar.text_input("3. Enter 6-digit OTP:", max_chars=6, type="password", key="owner_otp_input")
...
def book_home_delivery(service_type: str, address: str = "Not Provided") -> str:
    ...
    return f"Success! {customer_name}, your order for {service_type} is successfully scheduled. Please provide your review and rating from 1 to 5 stars for this item."
...
def process_payment(amount: float, payment_method: str) -> str:
    ...
    with open("payments.csv", mode="a", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(["Timestamp", "Customer Name", "Item", "Amount", "Method", "Transaction ID"])
        writer.writerow([timestamp, customer_name, item, amount, payment_method, txn_id])
        
    return f"Payment of ₹{amount} via {payment_method} processed successfully for {customer_name}! Transaction ID: {txn_id}."
...
def add_product_review(rating: int, review_comment: str) -> str:
    ...
    return f"Thank you so much, {customer_name}! Your review ({rating}/5 stars) for '{item}' has been saved successfully. We truly appreciate your feedback!"
...
if st.session_state.user_role == "Owner":
    ...
    st.write(f"Welcome, **{st.session_state.logged_in_user}**! You can search products, place orders, make payments, and write reviews here.")
    ...
    if user_prompt := st.chat_input("Type your question or request here (Supports all languages)..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("AI Agent is thinking..."):
                try:
                    context_memory = f" [Context - User: {st.session_state.logged_in_user}, Last item: {st.session_state.last_booked_item}]"
                    ...
                    response = client.models.generate_content(
                        ...
                        config=types.GenerateContentConfig(
                            ...
                            temperature=0.3,
                            system_instruction=(
                                "You are an advanced multi-lingual enterprise AI assistant. "
                                "1. Detect user language and ALWAYS respond in that same language. "
                                "2. Use tools accurately. "
                                "3. REVIEW RULE: Immediately after booking a home delivery, automatically ask the customer to provide a rating from 1 to 5 stars and a review comment. "
                                "4. THANK YOU RULE: When the customer provides their rating/review, use the 'add_product_review' tool and warmly say thanks to the customer."
                            )
                        ),
                    )
                    ...
                    if response.function_calls:
                        for function_call in response.function_calls:
                            ...
                            followup_prompt = (
                                ...
                                f"Now answer user request: '{user_prompt}' in the user's language naturally."
                            )
