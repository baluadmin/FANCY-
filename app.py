...
# Streamlit Page Configuration & Professional High-Contrast Styling CSS
st.set_page_config(
...
if not product_records:
    product_records = [
        {"id": "ITM001", "name": "Bluetooth Wireless Headset", "price": "1200", "stock": "50", "category": "Headset", "description": "High bass wireless audio", "image": ""},
        {"id": "ITM003", "name": "Fast Type-C Charger 33W", "price": "650", "stock": "120", "category": "Charger", "description": "Quick charge adapter", "image": ""}
    ]
...
if st.session_state.current_view == "Home":
    ...
    for idx, prod in enumerate(filtered_items):
        with st.container(border=True):
            ...
            q_val = st.number_input("Quantity", min_value=1, value=1, key=f"qty_{prod['id']}_{idx}")
            if st.button(f"Add to Cart - {prod['name']}", key=f"add_{prod['id']}_{idx}"):
                st.session_state.cart.append({"product": prod['name'], "quantity": f"{q_val} Units"})
